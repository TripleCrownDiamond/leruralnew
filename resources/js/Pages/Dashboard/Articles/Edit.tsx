...
import { ArrowLeft, CalendarClock, Code2, Eye, FileText, Newspaper, Sparkles, Trash2, Copy, Check } from 'lucide-react';
...
export default function Edit({ article, categories }: Props) {
    ...
    const isPublishedNow = Boolean(article.published_at && new Date(article.published_at).getTime() <= Date.now());
    const publicArticleUrl = route('article.show', article.slug);
                        ...
                        <>
                            ...
                            >
                                ...
                                Voir l'article
                                                        ...
                                                        {isPublishedNow && (
                                <AdminButton type="button" variant="secondary" icon={copiedLink ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />} onClick={copyArticleLink}>
                                    {copiedLink ? 'Lien copie' : 'Copier le lien'}
                                </AdminButton>
                            )}
